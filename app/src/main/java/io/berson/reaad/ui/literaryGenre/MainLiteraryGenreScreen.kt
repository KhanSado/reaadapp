...
fun MainLiteraryGenreScreen(navController: NavController, vm: LiteraryGenreViewModel) {
    ...
    Scaffold (
        ...
        floatingActionButton = {
            FloatingActionButton(navController = navController, DestinationScreen.CreateNewLiteraryGenreScreen.name)
        }
    ) {
        GradientSurface {
            LazyColumn(
                ...
                items(items = mountLiteraryGenreList(literaryGenreUiState.literaryGenreList), itemContent = { item ->
                    literaryGenreView(
                        name = item.name,
                        clickOnItem = {
